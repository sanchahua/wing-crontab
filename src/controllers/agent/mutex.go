...
func (queueMutex *QMutex) dispatch(msgId int64, id int64, address string, send sendFunc, c chan *SendData, success func(*runItem)) {
	queue := (*queueMutex)[id]
	...
	success(item)
	c <- newSendData(msgId, agent.CMD_RUN_COMMAND, sendData, /*node.AsyncSend*/send, item.id, item.isMutex)
}

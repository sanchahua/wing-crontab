...
func NewMutexController(
	...
	//最终落入这个api crontabMutexController.Add
	onCronChange OnCronChangeEventFunc,
	//这个事件由leader分发定时任务到节点，节点收到定时任务时触发
	//最终接收的api是crontabMutexController.ReceiveCommand
	onCommand OnCommandFunc,
	//addlog AddLogFunc,
) *MutexController {
	c      := &MutexController{
...
//func (c *MutexController) clientCheck() {
...
//func (c *MutexController) checkClientUniqueExists(runningChan chan *clientExists)  *clientExists {
...
func (c *MutexController) onClientEvent(tcp *agent.AgentClient, cmd int , content []byte) {
...
func (c *MutexController) onServerEvent(node *agent.TcpClientNode, event int, content []byte) {
	start :=time.Now()
...
// send data to leader
func (c *MutexController) SendToLeader(data []byte) {
...
//func (c *MutexController) OnPullCommand(node *agent.TcpClientNode) {
//	//log.Debugf("ou pull")
//	c.onPullChan <- node
//}

func (c *MutexController) Pull() {
	//log.Debugf("##############################pull command(%v)", agent.CMD_PULL_COMMAND)
	c.client.AsyncWrite(agent.Pack(agent.CMD_PULL_COMMAND, []byte("")))
}

//func (c *MutexController) setStatistics(id int64) {
//	c.statisticsLock.Lock()
//	st, ok := c.statistics[id]
//	if !ok {
//		st = &Statistics{}
...
//func (c *MutexController) setStatisticsTime(id int64) {
//	c.statisticsLock.Lock()
//	st, ok := c.statistics[id]
//	if ok {
//		current := int64(time.Now().UnixNano() / 1000000)
...
func (c *MutexController) sendService() {
...
//func (c *MutexController) getTimeout(id int64) int64 {
//	c.statisticsLock.Lock()
...
func (c *MutexController) Dispatch(id int64, command string, isMutex bool, addWaitNum func(), subwaitNum func() int64) {
...
func (c *MutexController) OnLeader(isLeader bool) {
	c.client.OnLeader(isLeader)
}

// start agent
func (c *MutexController) Start() {
	c.server.Start()
}

// close agent
func (c *MutexController) Close() {
	c.server.Close()
}
